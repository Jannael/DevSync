...
import { type CustomError, DatabaseError, NotFound } from '../../error/error'
...
const model = {
	list: async (
		...
		try {
			const allTasks = await dbModel
				...
				.lean<
					Array<
						Pick<ITask, '_id' | 'name' | 'priority' | 'isComplete' | 'user'>
					>
				>()
			...
			if (allTasks === null)
				throw new NotFound(
					'Group not found',
					'The group may not have any tasks',
				)

			const response: IListTask = allTasks.reduce<IListTask>(
				(acc, task) => {
					acc.task.push({
						_id: task._id,
						name: task.name,
	...
	create: async (task: Omit<ITask, '_id'>): Promise<Types.ObjectId> => {
		try {
	...
	delete: async (taskId: Types.ObjectId): Promise<boolean> => {
		try {

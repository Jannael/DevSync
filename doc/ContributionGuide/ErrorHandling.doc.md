...
export const CustomNotFound = CreateError<ICustomNotFound>(404, 'customNotFound');
```
...
```typescript
if (!group) {
    throw new NotFound('Group not found', 'The requested group ID does not exist in our database.');
}
```

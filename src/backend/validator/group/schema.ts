...
const schema = z.object({
  techLead: z.array(z.object({
    fullName: z.string('techLead fullName is required').min(3, { message: 'techLead fullName must be at least 3 characters long' }).max(255, { message: 'techLead fullName must be at most 255 characters long' }),
    account: z.string('techLead account is required').email()
  })).refine(
    (arr) => new Set(arr).size === arr.length,
    { message: 'The techLead array must contain only unique elements (no duplicates).' }
  ).optional(),
  ...
  member: z.array(z.object({
    ...
    fullName: z.string('member.fullName is required').min(3, { message: 'member.fullName must be at least 3 characters long' }).max(255, { message: 'member.fullName must be at most 255 characters long' }),

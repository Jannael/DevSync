...
const model = {
	get: async (id: Types.ObjectId): Promise<IGroup> => {
		try {
	...
	exists: async (
		...
		try {
			...
			if (techLeadAccount !== undefined) {
				if (!verifyEmail(techLeadAccount))
					throw new UserBadRequest(
						'Invalid credentials',
						`The account ${techLeadAccount} is invalid`,
					)
				const isTechLead = await dbModel.exists({
					_id: groupId,
					'techLead.account': techLeadAccount,
				})
	...
	create: async (
		...
		techLead: { fullName: string; account: string },
	): Promise<IGroup & Required<Pick<IGroup, '_id'>>> => {
		try {
			if (!verifyEmail(techLead.account))
				throw new UserBadRequest(
					'Invalid credentials',
					`The account ${techLead.account} is invalid`,
				)
			...
			if (data.techLead !== undefined) {
				for (const techLd of data.techLead) {
					...
					await UserModel.invitation.create(
						{ ...techLd, role: 'techLead' },
						{
							_id: created._id,
							name: created.name,
							color: created.color,
	...
	update: async (
		...
		try {
			...
			if (data.member !== undefined)
				throw new UserBadRequest(
					'Invalid credentials',
					'You can not change the member',
				)
			if (data.techLead !== undefined)
				throw new UserBadRequest(
					'Invalid credentials',
					'You can not change the techLead',
				)

			const group = await dbModel
				.findOne({ _id: groupId }, { _id: 1, name: 1, color: 1 })
				.lean()
			if (group === null)
				throw new NotFound(
					'Group not found',
			...
			if (updatedGroup.techLead !== undefined) {
				for (const techLead of updatedGroup.techLead) {
					await UserModel.group.update(techLead.account, groupId, {
			...
			if (updatedGroup.member !== undefined) {
				for (const member of updatedGroup.member) {
					await UserModel.group.update(member.account, groupId, {
						name: updatedGroup.name,
						color: updatedGroup.color,
					})
				}
	...
	delete: async (groupId: Types.ObjectId): Promise<boolean> => {
		try {
			if (!Types.ObjectId.isValid(groupId))
				throw new UserBadRequest('Invalid credentials', 'The _id is invalid')

			const members = await dbModel
				.findOne({ _id: groupId }, { member: 1, techLead: 1 })
				.lean()
			if (members === null)
				throw new NotFound(
					'Group not found',
	...
	member: {
		add: async (
			groupId: Types.ObjectId,
			member: NonNullable<IGroup['member']>[number],
		): Promise<boolean> => {
			try {
				if (!verifyEmail(member.account))
					throw new UserBadRequest(
						'Invalid credentials',
						`The account ${member.account} is invalid`,
					)
				if (!Types.ObjectId.isValid(groupId))
					throw new UserBadRequest('Invalid credentials', 'The _id is invalid')

				const currentMembers = await dbModel.findOne(
					{ _id: groupId },
					{ member: 1, _id: 0 },
				)
				...
				if (
					currentMembers?.member !== undefined &&
					currentMembers.member.find((m) => m.account === member.account) !==
						undefined
				)
					return true

				if (
					currentMembers?.member?.length !== undefined &&
				...
				errorHandler.allErrors(
					...
					new DatabaseError(
						'Failed to save',
						`the member with the account ${member.account} was not added`,
					),
				)
				throw new DatabaseError(
					'Failed to save',
		...
		remove: async (
			...
			try {
				...
				const isTechLead = await dbModel.findOne(
					{ _id: groupId, 'techLead.account': account },
					{ techLead: 1, _id: 0 },
				)
				if (isTechLead !== null && isTechLead !== undefined) {
					if (
						isTechLead.techLead?.length !== undefined &&
						isTechLead.techLead?.length <= 1
					)
						throw new Forbidden(
							'Access denied',
		...
		update: async (
			...
			try {
				if (!Types.ObjectId.isValid(groupId))
					throw new UserBadRequest('Invalid credentials', 'The _id is invalid')
				if (!verifyEmail(data.account))
					throw new UserBadRequest(
						'Invalid credentials',
						`The account ${data.account} is invalid`,
					)
				if (!verifyEmail(updateData.account))
					throw new UserBadRequest(
						'Invalid credentials',
						`The account ${updateData.account} is invalid`,
					)
		...
		exists: async (
			...
			try {
				...
				throw new UserBadRequest(
					'Invalid credentials',
					`The user with the account ${account} does not belong to the group`,
				)
			} catch (e) {
				errorHandler.allErrors(
					e as CustomError,
